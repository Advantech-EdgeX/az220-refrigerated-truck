...
namespace refrigerated_truck
{
	...
	class Program
	{
		...
		private static string LatLngToGPSddmm(string text)
		{
			string result = "";
			...
			if (ary.Length == 2) {
				...
				if (double.TryParse(ary[0], out degree) && double.TryParse(ary[1], out temp)) {
					...
					string mmdot = a[0];
					double d_mmdot = Convert.ToDouble(mmdot);
					string dotmm = a[1];
					result = Math.Abs(degree).ToString() + d_mmdot.ToString("00") + "." + dotmm;
				}
			}
			return result;
		}

		private static string GPSddmmToLatLng(string text)
		{
			string result = "";
			string[] ary = text.Trim().Split('.');
			double d_ddmm, d_dotmm;
			if (double.TryParse(ary[0], out d_ddmm) && double.TryParse(ary[1], out d_dotmm)) {
				string ddmm = d_ddmm.ToString();
				string dotmm = d_dotmm.ToString();
				int l = ddmm.Length;
				string dd = ddmm.Substring(0, l-2);
				string s_mmdot = ddmm.Substring(l-2, 2);
				double d_mmdot = Convert.ToDouble(s_mmdot);

				double input_double = Convert.ToDouble(text);
				double input_integer = Math.Floor(input_double);
				double input_decimal = input_double - input_integer;
				double mm = d_mmdot + input_decimal;
				mm = mm / 60;
				if (mm >= 1) {
					redMessage($"Error!!! It should be less than 1. {mm}");
					return result;
				}
				string[] s_ary_mm = mm.ToString().Split('.');
				string s_mm = s_ary_mm[1];
				result = dd + "." + s_mm;
			}
			return result;
		}
		...
		static void MessageNMEAprint(double lat, double lon)
		{
			string angle_lon="E", angle_lat="N";
			var dm_lat = LatLngToGPSddmm(lat.ToString());
			var dm_lon = LatLngToGPSddmm(lon.ToString());
			if (lat < 0) {
				angle_lat = "S";
			}
			if (lon < 0) {
			...
			string dtDatePatt = @"ddMMyy";
			string dtTimePatt = @"hhmmss";
			DateTime saveUtcNow = DateTime.UtcNow;
			string dtDateString = saveUtcNow.ToString(dtDatePatt);
			string dtTimeString = saveUtcNow.ToString(dtTimePatt);
			string nmea = "$GPRMC,"+dtTimeString+".000,A,"+dm_lat+","+angle_lat+","+dm_lon+","+angle_lon+",0.00,0.00,"+dtDateString+",,,A*";
			string ck = getChecksum(nmea);
			greenMessage($"{nmea}{ck}");
		}
		...
		static void UpdatePosition()
		{
			...
			greenMessage($"truckOnSection={truckOnSection} currentLat,currentLon={currentLat},{currentLon}");
			MessageNMEAprint(currentLat, currentLon);
		}
		...
		static void GetRoute(StateEnum newState)
		{
			...
			else
			{
				...
				redMessage($"Route found. Number of points = {nPoints}");
			...
			try {
				...
				if(e.Topic == MqttTopic[0]) {
					if (state != StateEnum.ready) {
						greenMessage($"Skip mqtt message state={state}");
						return;
					}
					...
					if (gEdgeXAIObj.Readings[0].Value.Contains("Longitude")) {
						...
						currentLat = (double)jsonOb["Latitude"];
						currentLon = (double)jsonOb["Longitude"];
						greenMessage($"currentLat,currentLon={currentLat},{currentLon}");
						MessageNMEAprint(currentLat, currentLon);
					} else {
						redMessage("Skip non latitude and longitude MQTT message.");
					}
				} else {
					greenMessage($"MQTT Received other topic \"{e.Topic}\", listening to \"{MqttTopic[0]}\".");
				}
			}
			catch (Exception ex) {
